...
import {DiscHeader, DiscContent} from '../common/textcontent';
import CustomButton from '../components/custombutton';

const DisclosureModal = props => {
  const [visible, setVisible] = useState(true);
  const [header, setHeader] = useState(DiscHeader);
  const [text, setText] = useState(DiscContent);

  const isProfessional = useRef(null);
  ...
  const getStorageData = async () => {
    isProfessional.current = await GetAsyncStorageData('U_isProfessional');
  };
