...
const DocumentScreen = props => {
  ...
  const getdocSavedStatus = async () => {
    const getIDStatus = await getisIDAdded();
    const getDID = await getisDIDAdded();
    const getAC = await getisACAdded();
    const getMC = await getisMCAdded();
    const getBC = await getisBCAdded();
    if (getIDStatus != null) {
        ...
        try {
          ...
          if (type === 'ID') {
            ...
            if (setIDDocsUri.current != null) {
              ...
              //  console.log('ID' + isIDAttached.current);
              await saveisIDUploaded('isDocsFileAdded', 1);
              setIDAttached(true);
              setisLoading(false);
              checkDocumentStatus();
            }
  ...
  const checkDocumentStatus = async () => {
    ...
    if (isCouple == 0 && isChildAdult == 0 && dependantData.current == null) {
      if (isIDAttached.current && isBankDocsAttached.current) {
        await saveisDocsAdded(1);
